...
import SalePrice from "../ships-log-common/SalePrice";
...
export default class Order extends React.Component {
  ...
  renderAcceptOfferButton(canAccept = true) {
    ...
    return (
      <button disabled={creatingOrder} onClick={sellAsset} className={`btn btn-success w-100`}>
  ...
  render() {
    ...
    return (
      <Card>
        {asset ? <AssetMetadata asset={asset} /> : <BundleMetadata bundle={assetBundle} />}
        ...
        <ul className="list-group list-group-flush">
          ...
          {errorMessage ? (
            ...
            <li className="list-group-item">
              {order.side === OrderSide.Buy ? this.renderAcceptOfferButton(isOwner) : null}
              {order.side === OrderSide.Sell ? this.renderBuyButton(!isOwner) : null}
            </li>
          )}
        </ul>
